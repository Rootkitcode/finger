...
})('n21pay_fing2', this, function () {
  'use strict'
  /**
   * @constructor
   * @param {Object=} options
   */
  var n21pay_fing2 = function (options) {
    if (!(this instanceof n21pay_fing2)) {
      return new n21pay_fing2(options)
    }

    var defaultOptions = {
      swfContainerId: 'Boofingerjs2',
      swbfpath: 'flash/compiled/FontList.swf',
      detectScreenOrientation: true,
      sortPluginsFor: [/palemoon/i],
      // To ensure consistent Boofingers when users rotate their mobile devices
      userDefinedFonts: [],
      ...
      // We require users to explicitly enable audio Boofingering on iOS 11.
  ...
  n21pay_fing2.prototype = {
    ...
    enumerateDevicesKey: function (keys, done) {
      ...
      .then(function (devices) {
        var enumerateDevicesbfp = []
        devices.forEach(function (device) {
          enumerateDevicesbfp.push('id=' + device.deviceId + ';gid=' + device.groupId + ';' + device.kind + ';' + device.label)
        })
        keys.addPreprocessedComponent({key: 'enumerate_devices', value: enumerateDevicesbfp})
        return done(keys)
      })
      .catch(function (e) {
        return done(keys)
    ...
    audioKey: function (keys, done) {
      ...
      if (AudioContext == null) {
        keys.addPreprocessedComponent({key: 'audio_bfp', value: null})
        return done(keys)
      }
      ...
      var audioTimeoutId = setTimeout(function () {
        console.warn('Audio Boofinger timed out.  with your user agent: "' + navigator.userAgent + '".')
        return done(keys)
      }, 1000)

      context.oncomplete = function (event) {
        clearTimeout(audioTimeoutId)
        var Boofinger = event.renderedBuffer.getChannelData(0)
                     .slice(4500, 5000)
        ...
        keys.addPreprocessedComponent({key: 'audio_bfp', value: Boofinger})
        return done(keys)
      }
    ...
    canvasKey: function (keys) {
      if (!this.options.excludeCanvas && this.isCanvasSupported()) {
        keys.addPreprocessedComponent({key: 'canvas', value: this.getCanvasbfp()})
      }
      return keys
    },
    webglKey: function (keys) {
      if (!this.options.excludeWebGL && this.isWebGlSupported()) {
        keys.addPreprocessedComponent({key: 'webgl', value: this.getWebglbfp()})
      }
      return keys
    },
    ...
    // flash fonts (will increase Boofingering time 20X to ~ 130-150ms)
    flashFontsKey: function (keys, done) {
      ...
      if (typeof this.options.swbfpath === 'undefined') {
        return done(keys)
      }
    ...
    getCanvasbfp: function () {
      ...
      ctx.fillStyle = '#069'
      // https://github.com/Valve/Boofingerjs2/issues/66
      ...
      if (canvas.toDataURL) { result.push('canvas bfp:' + canvas.toDataURL()) }
      return result.join('~')
    },

    getWebglbfp: function () {
      var gl
    ...
    getWebglVendorAndRenderer: function () {
      /* This a subset of the WebGL Boofinger with a lot of entropy, while being reasonably browser-independent */
      try {
    ...
    loadSwfAndDetectFonts: function (done) {
      var hiddenCallback = '___bfp_swf_loaded'
      ...
      window.swfobject.embedSWF(this.options.swbfpath, id, '1', '1', '9.0.0', false, flashvars, flashparams, {})
    },
